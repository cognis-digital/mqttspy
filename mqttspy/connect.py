"""Native cognis-connect emit for mqttspy — forward findings to any platform.

Maps mqttspy's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    mqttspy ... --format json | mqttspy-emit --to stix
    mqttspy-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('topic', '') + ' - ' + rec.get('payload', '')
        out['severity'] = 'info'
        out['type'] = 'mqtt_message'
        out['description'] = f"MQTT message on topic {rec.get('topic', '')}"
        out['tags'] = ['mqtt', 'message']
        out['ipv4'] = rec.get('client_ip')
        out['domain'] = ''
        out['url'] = ''
        out['sha256'] = ''
        out['cve'] = ''
