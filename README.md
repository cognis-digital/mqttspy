...
<img src="https://capsule-render.vercel.app/api?type=rect&color=0:6b46c1,100:2b6cb0&height=120&section=header&text=MQTTSPY&fontSize=48&fontColor=ffffff&fontAlignY=58" width="100%" alt="MQTTSPY"/>
...
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=3500&pause=1000&color=6B46C1&center=true&vCenter=true&width=720&lines=Passively+map+an+MQTT+broker+enumerate+topics+detect+unauthe;Self-hostable+%C2%B7+MCP-native+%C2%B7+CI-ready+%C2%B7+polyglot" width="720"/>

[![PyPI](https://img.shields.io/pypi/v/cognis-mqttspy.svg?color=6b46c1)](https://pypi.org/project/cognis-mqttspy/) [![CI](https://github.com/cognis-digital/mqttspy/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/mqttspy/actions) [![License: COCL 1.0](https://img.shields.io/badge/License-COCL%201.0-2b6cb0.svg)](LICENSE) [![Suite](https://img.shields.io/badge/Cognis-Neural%20Suite-6b46c1.svg)](https://github.com/cognis-digital)
...
Shodan-bait — 'I found 40k open brokers leaking door-lock state' is evergreen viral content; the tool turns that into a repeatable, redactable scan.

`mqttspy` is single-purpose, scriptable, and self-hostable: point it at a target, get prioritized results in the format your workflow already speaks (table · JSON · SARIF), gate CI on it, and let agents drive it over MCP.
...
**Topics:** `cognis` `energy` `critical-infrastructure` `privacy`
...
mqttspy scan . --fail-on high        # CI gate (non-zero exit)
```
...
```text
$ mqttspy scan .
  [HIGH    ] MQT-001  example finding             (./src/app.py)
  [MEDIUM  ] MQT-002  another signal              (./config.yaml)
...
`mqttspy` is interoperable with every popular way of using AI:

- **MCP server** — `mqttspy mcp` (Claude Desktop, Cursor, Cognis.Studio, [uncensored-fleet](https://github.com/cognis-digital/uncensored-fleet))
- **OpenAI-compatible / JSON** — pipe `mqttspy scan . --format json` into any agent or LLM
...
| | **Cognis mqttspy** | mqtt-explorer + nuclei |
|---|:---:|:---:|
...
*Built in the spirit of **mqtt-explorer + nuclei**, re-framed the Cognis way. Missing a credit? Open a PR.*
...
Pipes into your stack: **SARIF** for code-scanning, **JSON** for anything, an **MCP server** (`mqttspy mcp`) for AI agents, and a webhook forwarder for SIEM/Slack/Jira. See [`docs/INTEGRATIONS.md`](docs/INTEGRATIONS.md).
...
```bash
pip install "git+https://github.com/cognis-digital/mqttspy.git"    # pip (works today)
pipx install "git+https://github.com/cognis-digital/mqttspy.git"   # isolated CLI
uv tool install "git+https://github.com/cognis-digital/mqttspy.git" # uv
pip install cognis-mqttspy                                          # PyPI (when published)
docker run --rm ghcr.io/cognis-digital/mqttspy:latest --help        # Docker
brew install cognis-digital/tap/mqttspy                             # Homebrew tap
curl -fsSL https://raw.githubusercontent.com/cognis-digital/mqttspy/main/install.sh | sh
```

| Linux | macOS | Windows | Docker | Cloud |
|---|---|---|---|---|
| `scripts/setup-linux.sh` | `scripts/setup-macos.sh` | `scripts/setup-windows.ps1` | `docker run ghcr.io/cognis-digital/mqttspy` | [DEPLOY.md](docs/DEPLOY.md) (AWS/Azure/GCP/k8s) |
...
> ### ⭐ If `mqttspy` saved you time, **star it** — it genuinely helps others find it.
